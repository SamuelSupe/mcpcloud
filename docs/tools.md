...
Tencent `tencent.vpc.list_addresses` and `tencent.vpc.list_nat_gateways` directly list EIP and NAT instances in one exact configured region, without Resource Center. Use empty `params`, `page_size` up to 100 and the returned cursor. Both require one configured account and reuse their single-instance detail field projections, omitting raw IP addresses and embedded NAT rules. Cursors bind profile/account/region/operation; changing totals require restarting. Offset pagination is not a snapshot. These native lists do not replace the `resources` DSL or include private NAT gateways.

Tencent direct lists `tencent.cvm.list_instances`, `tencent.cdb.list_instances`, `tencent.redis.list_instances` and `tencent.tke.list_clusters` use their product APIs without Cloud Resource Center. Specify one exact configured `region`, empty `params`, and `page_size` up to 100; continue with `cursor`. They require one configured account and reuse existing detail mappings, including CVM IP addresses. Unknown fields and credentials are not forwarded. Cursors bind profile/account/region/operation; changing totals require restarting. Offset pagination is not a snapshot. These native lists do not replace the `resources` DSL.

Tencent `tencent.cos.list_buckets` directly calls COS GetService with one exact configured region, empty `params` and `page_size` up to 100. Continue using `cursor`; cursors bind profile/account/region/operation. The response owner and each bucket region must match the configured scope. Returns bucket ID/name, region and creation time, without objects or ACL/configuration reads. Requires GetService permission. Marker pagination is not a snapshot. This native list does not replace the `resources` DSL.

Tencent unified `resources` queries can opt into `options.resource_mode: direct` (default: `cloudrc`). Direct mode enumerates CVM, MySQL, Redis, TKE, CLB, EIP, NAT and COS through the native lists, one product API page per adapter call. It requires exactly one account and configured exact regions. It cannot be combined with `resource_view_id`. Coverage is limited to these eight product types; IAM still uses Resource Center. Existing detail projections apply, including CVM addresses. Use `fields id, service, region` to omit addresses. Filters, sorting and summaries remain engine-side. Provider errors remain errors; request/scan limits can truncate results. Pagination is not a snapshot. Existing resource-center native operations remain unchanged.
