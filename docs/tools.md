...
| Volcengine | `volcengine.rdsmysql.describe_db_instance_detail` → RDS MySQL `DescribeDBInstanceDetail`; `instance_id`: string | `volcengine.vke.list_clusters` → VKE `ListClusters` with `Filter.Ids:[cluster_id]`, page 1, size 1; `cluster_id`: string | Top-level `region` must be exact; profile must contain exactly one account. |
